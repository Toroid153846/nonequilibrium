...
\begin{frame}[fragile]{経路積分の定義}
    \begin{tikzpicture}
        %\draw (8,4) grid (8,4);
        \draw[->,thick] (0,0) -- (0,4) node[above] {$x$};
        \draw[->,thick] (0,0) -- (8.4,0) node[right] {$t$};
        \draw[thick] (2.4,0.8) -- (0,0.8) node[left]  {$x_0$};
        \draw[dashed] (2.4,3.2) -- (0,3.2) node[left]  {$x_1$};
        \draw[thick] (2.4,3.2) -- (4,3.2);
        \draw[dashed] (4,1.6) -- (0,1.6) node[left]  {$x_2$};
        \draw[thick] (4,1.6) -- (4.8,1.6);
        \draw[dashed] (7.2,2.4) -- (0,2.4) node[left]  {$x_K$};
        \draw[thick] (8,2.4) -- (7.2,2.4);
        \draw[dashed] (8,2.4) -- (8,0)   node[below] {$\tau$};
        \draw[dashed] (0,0.8) -- (0,0)   node[below] {$0$};
        \draw[dashed] (2.4,3.2) -- (2.4,0) node[below] {$t_1$};
        \draw[dashed] (4,3.2) -- (4,0)   node[below] {$t_2$};
        \draw[dashed] (4.8,0) -- (5.6,0) node[below] {$\cdots$};
        \draw[dashed] (7.2,4) -- (7.2,0) node[below] {$t_K$};
      \end{tikzpicture}      
\begin{align*}
    \sum_{K=0}^{\infty}\sum_{\lbrace x_k\rbrace_{k=0}^K}\sum_{\lbrace\nu_k\rbrace_{k=1}^K}\int_{0<t_1<\cdots<t_K<\tau}\\
\end{align*}
\end{frame}
\begin{frame}[fragile]{経路確率の定義}
    経路確率
    \begin{align}
        P[\bm{x}_\tau\mid\bm{\lambda}_\tau]
        =&P(x_0,0)\prod_{k=1}^K \left( \exp\left(-\int_{t_k}^{t_{k+1}}\gamma(x_k,t')\dd{t'}\right)R_{\nu_k}(x_k|x_{k-1};t_k) \dd{t_k}\right)\\
        &\times\exp\left(-\int_{t_K}^{\tau}\gamma(x_k,t')\dd{t'}\right) \\
...
\begin{frame}[fragile]{期待値と規格化}
経路に対応する物理量$A$の期待値
\begin{align*}
    \ev{A}=&\sum_{K=0}^{\infty}\sum_{\lbrace x_k\rbrace_{k=0}^K}\sum_{\lbrace\nu_k\rbrace_{k=1}^K}\int_{0<t_1<\cdots<t_K<\tau}A[\bm{x}_\tau,\bm{\lambda}_\tau]\\
